...
- [5.2 그래디언트에 대한 간단한 표현식과 해석(Simple expressions and interpretation of the gradient)](#5.2)
- [5.3 체인룰을 결합한 표현식(Compound expressions with chain rule)](#5.3)
...
- [5.8 벡터 연산을 위한 그래디언트(Gradients for vectorized operations)](#5.8)
- [5.9 요약(Summary)](#5.9)
...
## 5.2 그래디언트에 대한 간단한 표현식과 해석(Simple expressions and interpretation of the gradient)
...
## 5.3 체인룰을 결합한 표현식(Compound expressions with chain rule)
...
### 5.5.1 구현에의 팁: 단계별 역전파(Implementation protip: staged backpropagation)
...
### 5.6.1 순전파 변수들을 저장(Cache forward pass variables)
...
### 5.6.2 포크에서 그래디언트를 합함(Gradients add up at forks)
...
## 5.8 벡터 연산을 위한 그래디언트(Gradients for vectorized operations)
...
### 5.8.1 행렬간 곱 그래디언트(Matrix-Matrix multiply gradient)
...
### 5.8.2 작고, 명확한 데이터로 작업하라(Work with small, explicit examples)
