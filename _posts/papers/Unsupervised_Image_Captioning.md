...
show-avatar: true
category: study
tags: [CVPR, CVPR2019, Unsupervised, Image Captioning, Adversarial Learning, GAN]
comments: true
---

# 목차

[***서론***](https://sangminwoo.github.io/Unsupervised_Image_Captioning#서론)

[***관련 연구***](https://sangminwoo.github.io/Unsupervised_Image_Captioning#관련 연구)

[***모델에 대한 설명***](https://sangminwoo.github.io/Unsupervised_Image_Captioning#모델에 대한 설명)

[***정리***](https://sangminwoo.github.io/Unsupervised_Image_Captioning#정리)


# 서론

현재까지의 이미지 캡셔닝 모델은 모두 supervised version이었습니다. 이 논문에서는 최초로(?) *unsupervised version*의 이미지 캡셔닝 모델을 제안합니다. 심지어 image set, sentence corpus, visual concept detector(본 논문에서는 object detector를 visual concept detector로 지칭함)를 거의 필요로 하지 않는다고 합니다. 캡션이 이미지의 함축적인 의미를 잘 내포하기 위해서 이미지와 캡션을 같은 latent space에 project하여 서로 reconstruct하는 방식을 사용합니다. 잘 이해가 가지 않지만 아래에 설명을 통해 차근차근 이해해봅시다.
...
![1](../../img/papers/Unsupervised_Image_Captioning/1.png)
...
* **Image captioning**: 기존의 이미지 캡셔닝 모델은 CNN을 이미지 인코더로, RNN을 문장 디코더로 사용하였습니다. 이 모델은 입력 이미지에 대한 ground truth 캡션을 생성할 확률을 높이는 방향으로 학습이 됩니다. Image-caption pair 데이터를 수집하는데 많은 비용이 드는 만큼 unsupervised를 지향하는 모델들이 많이 제안되어 왔지만, 이전까지 제안되었던 모델들은 모두 image-sentence pair을 많든 적든 필요로 합니다.

* **Unsupervised Machine Translation**: source language와 target language가 동일한 latent space에 mapping되기 때문에 다른 언어라도 같은 의미를 지닌 문장들은 비슷한 위치에 놓이게 됩니다. 따라서 translation이 가능하게 됩니다.
...
![2](../../img/papers/Unsupervised_Image_Captioning/2.png)
...
![3](../../img/papers/Unsupervised_Image_Captioning/3.png)
...
\\p_t\\: probability over the dictionary at the \\t\\-th time step
...
![4](../../img/papers/Unsupervised_Image_Captioning/4.png)
...
![5](../../img/papers/Unsupervised_Image_Captioning/5.png)
...
![8](../../img/papers/Unsupervised_Image_Captioning/8.png)

---

Generator에게 주어지는 *image reconstruction reward*는 image reconstruction loss에 반대 부호를 취한것으로 여길 수 있습니다. 


2) Sentence reconstruction
Discriminator를 통해 인코딩된 문장을 common latent space에 project할 수 있습니다. 이는 다른 관점에서 image representation으로 볼 수 있고, 이를 이용하여 Generator에서 기존의 문장을 reconstruct할 수 있습니다. 이러한 과정을 통해 latent space상의 이미지와 문장을 aligning함과 동시에 image representation로부터 문장을 어떻게 decode할 것인지를 학습합니다.

***sentence reconstruction loss***

---

![9](../../img/papers/Unsupervised_Image_Captioning/9.png)
...
![10](../../img/papers/Unsupervised_Image_Captioning/10.png)

---

Generator를 먼저 살펴봅시다. Generator에서 샘플링 연산은 미분불가능한 연산이므로, policy gradient(+=+)을 이용하여 학습할 수 있습니다. 또한 역전파를 통해 sentence reconstruction loss의 그래디언트가 Generator에 전달됩니다. 이 두가지 그래디언트가 지속적으로 Generator를 업데이트 합니다.

---

![11](../../img/papers/Unsupervised_Image_Captioning/11.png)

\\theta\\: trainable parameters in the generator

\\gamma\\: decay factor

\\b_t\\: baseline reward estimated using *self-critic[1]*
...
![12](../../img/papers/Unsupervised_Image_Captioning/12.png)

---

## 초기화
본 논문에서는 Generator와 Discriminator를 pre-train하는 initialization pipeline을 제시합니다. Generator에서 생성되는 pseudo caption을 이용하여 모델을 초기화 합니다. 단계는 다음과 같습니다.
...
![13](../../img/papers/Unsupervised_Image_Captioning/13.png)

---

논문에서 기여한 바를 아래와 같이 정리할 수 있습니다.

1. Image captioning task에 있어 unsupervised 방식을 최초로 제안한 논문으로써 의의가 있습니다.

2. 세가지 학습 목표를 명확히 보여줍니다.
- 생성된 캡션이 말뭉치의 실제 문장과 분간되지 않도록 한다.
- 모델은 이미지의 object 정보를 담아야 한다.
- 이미지와 문장의 feature을 동일한 latent space상에 정렬하여 bi-directional reconstruction을 수행한다.

3. Unlabeled 데이터를 이용하여 모델을 초기화하는 pipeline을 제시하였습니다.

4. Shutterstock에서 2백만 개 이상의 문장을 크롤링하였습니다.