...
---------------------------
CRF은 레이블의 인접성에 대한 정보를 바탕으로 레이블을 추측하는 모델이다. 데이터 \\(X\\)가 주어졌을 때 레이블 \\(Y\\)를 구하는 일을 생각해보자.가령, image segmentation에서는 데이터는 픽셀의 값(RGB 히스토그램)이 되고 레이블은 각 픽셀의 클래스(잔디, 하늘, 물, 소)가  될 것이다. text에서는 데이터는 각 문자의 단어가 되고 레이블은 그 단어의 클래스(사람, 장소, 조직)이 된다. 이러한 테스크를 기존의 방법(나이브 베이즈)을 이용하면 feature들이 서로 독립적으로 표현되므로 그들 간의 관련성(correlation)이 무시된다. 서로의 관련성을 표현하기 위해 각각의 feature를 node로 간주하고 edge로 연결하게 되면 네트워크가 굉장히 복잡해지게 된다. 반면, CRF는 입력변수 사이의 관련성에 대해 신경쓰지 않아도 된다. 즉, \\(P(X)\\)를 명시적으로 모델링할 필요 없이 주어진 \\(X\\)를 통해 \\(Y\\)의 확률 분포를 모델링한다. (\\(P(X,Y)\\)가 아닌 \\(P(Y|X)\\)를 모델링한다.) 예컨대 저스틴 비버의 하루 일과를 찍어둔 여러장의 스냅샷이 있다고 하자. 당신은 그 사진들에 대해서 어떤 활동을 하고 있는지(먹는다, 잔다, 운전한다 등)를 레이블링해야 한다. 한가지 방법은 그저 한 장의 이미지만을 보고 레이블링을 하는 것이다. 가령, 어두운 이미지면 잠을 잔다거나, 밝은 이미지면 춤을 춘다고 할 수 있을 것이다. 그러나 연속적인 속성을 무시한채로 분류를 하게 되면 많은 정보를 잃게 된다. 그렇다면 분류해야하는 사진 직전에 찍은 사진을 보고 레이블링을 할 수 있다면 어떨까? 예를 들어, 그 사진이 입을 크게 벌리고 있는 사진이라면, 현재 사진은 춤을 추고 있다보다는 하품을 하고있다 혹은 먹고있다로 분류하는게 타당할 것이다. 즉, 이와 같이 labeling의 정확도를 높이기 위해 인접 정보를 활용하는 것이 CRF 모델이 하는일이다. 
...
---------------------------
데이터 \\(X\\)가 주어졌을 때 레이블 \\(Y\\)가 나타날 조건부확률 \\(P(Y|X)\\)를 직접적으로 반환하는 모델이다. 레이블 정보가 있어야 하기 때문에 지도학습(supervised learning) 범주에 속하며 \\(X\\)의 레이블을 잘 구분하는 결정경계(decision boundary)를 학습하는 것이 목표가 된다. discriminative model은 generative model에 비해 가정이 단순하고, 학습데이터 양이 충분하다면 좋은 성능을 내는것으로 알려져 있다. 선형회귀(linear regression), 로지스틱회귀(logistic regression)은 discriminative model의 대표적인 예이다.
