...
본 논문에서 저자는 최초로 Visual Storytelling을 위한 벤치마크 데이터셋을 제시합니다. 기존의 Image Captioning을 vision-to-language task라고 한다면 Visual Storytelling은 **sequential vision-to-language task**라고 일컫을 수 있습니다. 즉, 연속성이 있는 이미지들(sequential vision)을 가지고 하나의 스토리(language)를 만드는 일입니다. 
